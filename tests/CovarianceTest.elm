...
testSuite : Test
testSuite =
    describe "Eigenbasis for a real symmetric matrix"
        [ diagonalSuite
        , nonDiagonalSuite
...
diagonalSuite =
    ...
    in
        describe "Off-diagonal elements are zero"
            [ test "x component should be non-zero" <|
                \() ->
                    notEqual (Vector.vector 0 0 0)
                        eigenbasis.x
...
nonDiagonalSuite =
    let
        ...
        eigenbasis =
            Covariance.eigenbasis matrix
    in
        describe "Off-diagonal elements are nonzero"
...
assertBasisContains : Vector -> Basis -> Expectation
...
basisToOrientationSuite =
    describe "converting an orthonormal basis to an axis-angle rotation"
